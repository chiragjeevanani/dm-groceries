              ...
              <BlurFade key={i} delay={0.1 + i * 0.05}>
                <MagicCard
                  ...
                  <div className="flex items-center gap-2.5 p-2.5 relative z-10">
                    <div
                      className={cn(
                        "h-9 w-9 rounded-full flex items-center justify-center transition-transform group-hover:scale-105 duration-500 shadow-sm",
                        stat.bg,
                        stat.color,
                      )}>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between px-4">
                  <div className="flex items-center">
                    {tabs.map((tab) => (
                      <button
                        ...
                        className={cn(
                          "relative py-3 px-4 text-xs font-semibold uppercase tracking-wider transition-all duration-300",
                          activeTab === tab
                            ? "text-[#1A4516] bg-white/50 font-bold"
                        ...
                        {activeTab === tab && (
                          <motion.div
                            ...
                            className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#1A4516] rounded-t-full mx-4"
                          />
                        )}
                      </button>
                    ))}
                  ...
                  <div className="py-2 lg:py-0 w-full lg:w-64">
                    <div className="relative group">
                      <HiOutlineMagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400 group-focus-within:text-[#1A4516] transition-all" />
                      <input
                        type="text"
                        placeholder="Search ID or Partner..."
                        className="w-full pl-9 pr-3 py-1.5 bg-slate-100/50 border-none rounded-lg text-xs font-semibold text-slate-700 placeholder:text-slate-400 focus:ring-1 focus:ring-[#1A4516]/10 transition-all outline-none"
                        value={searchTerm}
                        onChange={(e) => {
                ...
                <AnimatePresence mode="popLayout">
                  {paginatedDeliveries.map((dlv, idx) => (
                    <motion.div
                      ...
                      className="group relative bg-white rounded-lg border border-slate-100 p-1.5 hover:shadow-xl hover:shadow-[#1A4516]/5 hover:border-[#1A4516]/20 transition-all duration-300 min-w-0">
                      <div className="flex flex-col lg:flex-row items-stretch gap-1">
                        {/* Partner Info Section */}
                        <div className="lg:w-48 p-2 bg-slate-50/50 rounded-lg border border-transparent group-hover:bg-[#1A4516]/[0.02] group-hover:border-[#1A4516]/5 transition-all min-w-0">
                          <div className="flex items-center gap-2.5">
                            <div className="relative shrink-0">
                              ...
                              <div className="absolute -bottom-1 -right-1 h-3.5 w-3.5 bg-[#1A4516] rounded-sm border-[1px] border-white flex items-center justify-center text-white text-[7px] font-bold shadow-sm">
                                {dlv.deliveryBoy.rating}
                              </div>
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-[8px] font-semibold text-[#1A4516] uppercase tracking-[0.1em] mb-0.5">
                                Partner
                              </p>
                              <h3 className="text-xs font-semibold text-slate-800 leading-none truncate">
                                {dlv.deliveryBoy.name}
                              </h3>
                              <a
                                href={`tel:${dlv.deliveryBoy.phone}`}
                                className="inline-flex items-center gap-1 mt-1 text-[9px] font-semibold text-slate-500 hover:text-[#1A4516] transition-colors"
                              >
                        ...
                        <div className="flex-1 p-2 flex flex-col justify-between min-w-0">
                            ...
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2 mb-0.5">
                                ...
                                <Badge
                                  ...
                                  className="text-[7px] font-semibold px-1.5 py-0 rounded-full uppercase tracking-wider shrink-0"
                                >
                                  {dlv.status}
                                </Badge>
                              </div>
                              <h4 className="text-[10px] font-semibold text-slate-500 flex items-center gap-1 flex-wrap">
                                <HiOutlineUser className="h-3 w-3 text-slate-400 shrink-0" />
                                <span className="text-slate-700 capitalize font-semibold">{dlv.customerName}</span>
                              </h4>
                            </div>
                            <div className="sm:text-right shrink-0">
                              <p className="text-[8px] font-semibold text-slate-400 uppercase tracking-wider leading-none">
                                Timing
                              </p>
                              <p className="text-[10px] font-semibold text-[#1A4516] tracking-tight mt-0.5">
                                {dlv.startTime || "—"}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="bg-slate-50/30 px-3 py-1.5 rounded-md border border-slate-100/30 min-w-0">
                            <p className="text-[10px] font-semibold text-slate-600 leading-tight truncate">
                              <HiOutlineMapPin className="inline h-2.5 w-2.5 text-[#1A4516] mr-1 -mt-0.5" />
                              {dlv.address}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="lg:w-16 flex items-center justify-center p-2 sm:p-3 shrink-0">
                          <button className="h-10 w-10 lg:h-full lg:w-full bg-slate-800 group-hover:bg-[#1A4516] rounded-lg lg:rounded-r-lg lg:rounded-l-none flex items-center justify-center text-white transition-all duration-500 shadow-xl shadow-slate-800/10 hover:shadow-[#1A4516]/20">
